...
% remove NaNs
nan_idx = isnan(x) | isnan(trials) | any(isnan(n), 2);
x(nan_idx) = [];
trials(nan_idx) = [];
n(nan_idx, :) = [];
...
ll = zeros(ops.bins, length(x));
if ~isempty(ops.custom)
    [decoded, ll] = decode(x(~ops.custom), n(~ops.custom, :), n(ops.custom, :), ops);
    x = x(ops.custom);
elseif ops.cv
    for k = unique(trials')
        loo = trials == k;
        [decoded(loo), ll(:, loo)] = decode(x(~loo), n(~loo, :), n(loo, :), ops);
    end
else
    [decoded, ll] = decode(x, n, n, ops);
end
...
sem = accumarray(x, err, [ops.bins, 1], @(x) std(x, 'omitnan') ./ sqrt(sum(~isnan(x))));
...
function [decoded, ll] = decode(x, train, test, ops)
